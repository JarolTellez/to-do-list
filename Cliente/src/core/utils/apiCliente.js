import { renovarAccessToken } from '../servicios/auth.js';
import { setAccessToken, getAccessToken, eliminarAccessToken } from "../accessTokenEstado.js";
...
export async function fetchConRenovacionAccessToken(url, opciones = {}) {
    ...
    if (!accessToken) {
        try {
            const data = await renovarAccessToken();
    ...
    while (reintentos <= MAX_REINTENTOS) {
        try {
            ...
            if (response.status === 401 && reintentos < MAX_REINTENTOS) {
                ...
                try {
                    const dataRenovacion = await renovarAccessToken();
