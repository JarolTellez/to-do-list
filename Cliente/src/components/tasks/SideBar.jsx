import React, { memo } from "react";

/**
 * Sidebar component for task filtering and statistics
 * @component Sidebar
 * @description Provides filtering options and task statistics display
 * @param {Object} props - Component properties
 * @param {Object} props.stats - Task statistics object
 * @param {string} props.filter - Current status filter
 * @param {string} props.priorityFilter - Current priority filter
 * @param {string} props.dateFilter - Current date filter
 * @param {Function} props.onFilterChange - Status filter change callback
 * @param {Function} props.onPriorityFilterChange - Priority filter change callback
 * @param {Function} props.onDateFilterChange - Date filter change callback
 * @param {Function} props.onRefresh - Refresh data callback
 * @param {Function} props.onClearAll - Clear all filters callback
 * @param {boolean} props.hasActiveFilters - Whether any filters are active
 * @param {boolean} props.isMobileOpen - Mobile sidebar open state
 * @param {Function} props.onMobileClose - Mobile close callback
 * @returns {JSX.Element} Sidebar interface
 */
const Sidebar = memo(
  ({
    stats,
    filter,
    priorityFilter,
    dateFilter,
    onFilterChange,
    onPriorityFilterChange,
    onDateFilterChange,
    onRefresh,
    onClearAll,
    hasActiveFilters,
    isMobileOpen,
    onMobileClose
  }) => {
    return (
      <>
        {/* Overlay para móvil */}
        {isMobileOpen && (
          <div 
            className="sidebar-mobile-overlay mobile-open"
            onClick={onMobileClose}
          />
        )}
        
        <aside className={`sidebar ${isMobileOpen ? 'mobile-open' : ''}`}>
          {/* Botón de cerrar en móvil */}
          {isMobileOpen && (
            <button
              className="sidebar-mobile-close"
              onClick={onMobileClose}
              aria-label="Cerrar menú"
            >
              ×
            </button>
          )}

          <div className="sidebar-actions">
            <button
              onClick={onRefresh}
              className="action-btn refresh-action"
              title="Sincronizar con base de datos"
            >
              Actualizar BD
            </button>

            {hasActiveFilters && (
              <button
                onClick={onClearAll}
                className="action-btn clear-action"
                title="Limpiar todos los filtros"
              >
                Limpiar Filtros
              </button>
            )}
          </div>

          <div className="filters">
            <h3>Filtrar Tareas</h3>

            <div className="filter-item">
              <label>Estado</label>
              <div className="filter-container">
                <button
                  type="button"
                  className={`toggle-btn ${filter === "pending" ? "active" : ""}`}
                  onClick={() => onFilterChange("pending")}
                >
                  Pendientes
                </button>
                <button
                  type="button"
                  className={`toggle-btn ${
                    filter === "completed" ? "active" : ""
                  }`}
                  onClick={() => onFilterChange("completed")}
                >
                  Completadas
                </button>
                <button
                  type="button"
                  className={`toggle-btn ${filter === "overdue" ? "active" : ""}`}
                  onClick={() => onFilterChange("overdue")}
                >
                  Vencidas
                </button>
              </div>
            </div>

            <div className="filter-item">
              <label>Ordenar por Prioridad</label>
              <div className="filter-container">
                <button
                  type="button"
                  className={`toggle-btn ${
                    priorityFilter === "highest" ? "active" : ""
                  }`}
                  onClick={() => onPriorityFilterChange("highest")}
                >
                  Mayor
                </button>
                <button
                  type="button"
                  className={`toggle-btn ${
                    priorityFilter === "lowest" ? "active" : ""
                  }`}
                  onClick={() => onPriorityFilterChange("lowest")}
                >
                  Menor
                </button>
              </div>
            </div>

            <div className="filter-item">
              <label>Filtrar por Fecha</label>
              <div className="filter-container">
                <button
                  type="button"
                  className={`toggle-btn ${
                    dateFilter === "today" ? "active" : ""
                  }`}
                  onClick={() => onDateFilterChange("today")}
                >
                  Para Hoy
                </button>
                <button
                  type="button"
                  className={`toggle-btn ${
                    dateFilter === "upcoming" ? "active" : ""
                  }`}
                  onClick={() => onDateFilterChange("upcoming")}
                >
                  Próximas
                </button>
              </div>
            </div>
          </div>

          <div className="statistics">
            <h3>Estadísticas</h3>
            <div className="statistic-item">
              <p>Total</p>
              <p>{stats.total}</p>
            </div>
            <div className="statistic-item completed-statistics">
              <p>Completadas</p>
              <p>{stats.completedCount}</p>
            </div>
            <div className="statistic-item pending-statistics">
              <p>Pendientes</p>
              <p>{stats.pendingCount}</p>
            </div>
            <div className="statistic-item overdue-statistics">
              <p>Vencidas</p>
              <p>{stats.overdueCount || 0}</p>
            </div>
          </div>
        </aside>
      </>
    );
  },
  (prevProps, nextProps) => {
    return (
      prevProps.filter === nextProps.filter &&
      prevProps.priorityFilter === nextProps.priorityFilter &&
      prevProps.dateFilter === nextProps.dateFilter &&
      prevProps.hasActiveFilters === nextProps.hasActiveFilters &&
      prevProps.isMobileOpen === nextProps.isMobileOpen &&
      prevProps.stats.total === nextProps.stats.total &&
      prevProps.stats.completedCount === nextProps.stats.completedCount &&
      prevProps.stats.pendingCount === nextProps.stats.pendingCount &&
      prevProps.stats.overdueCount === nextProps.stats.overdueCount &&
      prevProps.onFilterChange === nextProps.onFilterChange &&
      prevProps.onPriorityFilterChange === nextProps.onPriorityFilterChange &&
      prevProps.onDateFilterChange === nextProps.onDateFilterChange &&
      prevProps.onRefresh === nextProps.onRefresh &&
      prevProps.onClearAll === nextProps.onClearAll &&
      prevProps.onMobileClose === nextProps.onMobileClose
    );
  }
);

Sidebar.displayName = "Sidebar";

export default Sidebar;